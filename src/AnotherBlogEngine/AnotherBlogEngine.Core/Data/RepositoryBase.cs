    ...
    public abstract class RepositoryBase<T> : IBaseRepository<T> where T : IEntity, new()
    {
        ...
        private string BuildInsertStatement()
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (computedAttributes.Length != 0) continue; //don't want to generate inserts for computed attributes
        ...
        private static string BuildSelectStatement()
        {
            ...
            {
                var computedAttributes = property.GetCustomAttributes(typeof(ComputedAttribute), true);
                if (computedAttributes.Length != 0) continue; //don't want to generate select for computed attributes
        ...
        private (string, object) BuildUpdateStatementAndParameters(T storedItem, T updatedItem)
        {
            ...
            {
                var computedAttributes = property.GetCustomAttributes(typeof(ComputedAttribute), true);
                if (computedAttributes.Length != 0) continue;  //don't want to generate updated for computed attributes
        ...
        private static object BuildParameterObjectForInsert(T item)
        {
            ...
            foreach (var propertyInfo in properties)
            {
                ...
                if (computedAttributes.Length != 0) continue; //we don't want to add params for a computed property
